...
from app1.models import logintable,usertable,personaltable,educationtable,medicaltable,accounttable,culturaltable
...
def culture(request):
    if request.method == "POST":
        ...
        g=request.POST["name7"]
        h = request.POST["name8"]
        i = culturaltable(Name=a, residence=b, language=c, ethnicity=d, practices=e, festivals=f,clothing=g,additional=h)
        i.save()
        return redirect("submit")
    return render(request,"cultureform.html")

def culturedata(request):
    x = usertable.objects.get(id=request.session["member_id"])
    print(x.Name)
    c = culturaltable.objects.filter(Name=x.Name)

    if request.method == "POST":
        c.Name = request.POST["name1"]
        c.residence = request.POST["name2"]
        c.language = request.POST["name3"]
        c.ethnicity = request.POST["name4"]
        c. practices = request.POST["name5"]
        c.festivals = request.POST["name6"]
        c.clothing = request.POST["name7"]
        c.additional = request.POST["name8"]

        c.save()

    return render(request, "culturedata.html", {"culturekey": c})

def cultureedit(request,id22):
    c = culturaltable.objects.get(id=id22)
    if request.method == "POST":
        c.Name = request.POST["name1"]
        c.residence = request.POST["name2"]
        c.language = request.POST["name3"]
        ...
        c.save()
        return redirect('update')
    return render(request,"cultureedit.html", {'key22':c})

def cultdelete(request,id12):
    c=culturaltable.objects.get(id=id12)
    c.delete()
    return redirect('deleted')
